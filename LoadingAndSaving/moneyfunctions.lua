...
function addMoney( amount )
    playerGold = playerGold + amount
    statistics.gold = statistics.gold + amount
...
function showCurrency( group )
    ...
    local goldIcon = display.newImage( imagesheet, spritedata:getFrameIndex( IMG_GOLD ) )
    goldIcon.anchorX, goldIcon.anchorY = 0, 0
    group:insert( goldIcon )
    goldIcon.x, goldIcon.y = 4, 4
    
    -- Add a text box
    goldText = display.newText( group, playerGold, 28, 4, 100, 20, native.systemFont, 18 )
